...
class ObjCClassMethod (object):
	...
	def __init__(self, cls, sel_name):
		self.cls = cls
		self.sel_name = sel_name
		method = c.class_getClassMethod(self.cls.ptr, sel(sel_name))
		if method:
			self.method = method
		else:
			raise RuntimeError('No (class) method found for selector "%s"' % (sel_name))
	...
	def __call__(self, *args, **kwargs):
		...
		if restype and issubclass(restype,ctypes.Structure):
		   send=c.objc_msgSend_stret
		else:
		   send=c.objc_msgSend
		send.argtypes = argtypes
		send.restype = restype
		res = send(self.cls.ptr, sel(self.sel_name), *args)
...
class ObjCInstanceMethod (object):
	...
	def __init__(self, obj, sel_name):
		self.obj = obj
		self.sel_name = sel_name
		self._objc_class = c.object_getClass(obj.ptr)
		method = c.class_getInstanceMethod(self._objc_class, sel(sel_name))
		if method:
			self.method = method
		else:
			raise RuntimeError('No method found for selector "%s"' % (sel_name))
	...
	def __call__(self, *args, **kwargs):
		...
		send.argtypes = argtypes
		send.restype = restype
		res = send(self.obj.ptr, sel(self.sel_name), *args)
